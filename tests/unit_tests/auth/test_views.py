...
class TokenViewTest(TestCase):
    def setUp(self):
        ...
        self.mock_token_helper = Mock(create=Mock(return_value=self.token_mock))
        self.request_mock = Mock(get_json=Mock(return_value={}))
        self.schema_mock = Mock()
        self.view = TokenView(self.mock_dao, self.mock_token_helper, self.schema_mock,
                              self.request_mock)
    ...
    def test_post_valid_data_user_not_exist(self):
        ...
        assert result == ({"error": "Invalid Credentials"}, HTTPStatus.HTTP_400_BAD_REQUEST)
